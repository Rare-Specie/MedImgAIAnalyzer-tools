...
static void print_usage() {
    std::cout << "Usage: npz_to_glb --input-dir <dir> --out <out.glb> [options]\n"
              << "Options:\n"
              << "  --raw-key <key>         Force raw array key\n"
              << "  --ann-key <key>         Force annotation key\n"
              << "  --ann-threshold <float> Threshold for annotation mask (default 0.5)\n"
              << "  --use-raw-threshold     If no annotation, threshold raw to build mesh\n";
}
...
static bool natural_less(const fs::path& a, const fs::path& b) {
...
static size_t compute_index(const std::vector<size_t>& idx,
...
static float read_value_as_float(const cnpy::NpyArray& arr, size_t idx) {
...
static const cnpy::NpyArray* find_array_by_key(const cnpy::npz_t& npz,
...
static const cnpy::NpyArray* find_array_by_keys(const cnpy::npz_t& npz,
...
static void squeeze_shape(std::vector<size_t>& shape) {
...
static bool extract_2d(const cnpy::NpyArray& arr,
...
static bool load_slices(const Args& args,
    ...
    if (files.empty()) {
        std::cerr << "No .npz files found in: " << args.input_dir << "\n";
        return false;
    }
    ...
    for (const auto& file : files) {
        ...
        if (!args.raw_key.empty()) {
            raw_arr = find_array_by_key(npz, args.raw_key);
        }
        if (!args.ann_key.empty()) {
            ann_arr = find_array_by_key(npz, args.ann_key);
        }
        if (!raw_arr) {
        ...
        if (!raw_arr) {
            std::cerr << "No raw array found in " << file << "\n";
            return false;
        }

        raw_slice.clear();
        if (!extract_2d(*raw_arr, raw_slice, slice_h, slice_w)) {
            std::cerr << "Failed to extract 2D raw from " << file << "\n";
            return false;
        }
        ...
        if (raws.empty()) {
            ...
            std::cerr << "Slice size mismatch in " << file << "\n";
            return false;
        }
        ...
        if (ann_arr) {
            ...
            if (!extract_2d(*ann_arr, ann_slice, ah, aw)) {
                std::cerr << "Failed to extract 2D ann from " << file << "\n";
                return false;
            }
            if (ah != height || aw != width) {
                std::cerr << "Annotation size mismatch in " << file << "\n";
                return false;
            }
            has_ann = true;
        } else {
...
static std::vector<unsigned char> build_texture_from_raw(const std::vector<float>& raw_volume,
                                                         size_t z_count,
                                                         size_t height,
                                                         size_t width) {
...
static void update_minmax(MeshData& mesh, float x, float y, float z, bool init) {
    if (init) {
...
static MeshData build_mesh_from_scalar(const std::vector<float>& vol,
                                       size_t z_count,
                                       size_t height,
                                       size_t width) {
    MeshData mesh;
...
static std::vector<float> build_raw_threshold_mask(const std::vector<float>& raw_volume) {
...
static size_t append_aligned(std::vector<unsigned char>& buffer,
                             const void* data,
                             size_t bytes,
                             size_t align = 4) {
...
static bool write_glb(const std::string& output_path,
                      const std::vector<PrimitiveData>& primitives,
                      const std::vector<unsigned char>& png) {
    if (primitives.empty()) {
        std::cerr << "No mesh generated. Nothing to write.\n";
        return false;
    }
    ...
    if (include_texture && png.empty()) {
        std::cerr << "PNG texture encode failed.\n";
        return false;
    }
    ...
    for (size_t p = 0; p < primitives.size(); ++p) {
        ...
        if (mesh.positions.empty() || mesh.indices.empty()) {
            std::cerr << "Empty mesh at primitive index " << p << ".\n";
            return false;
        }
    ...
    size_t json_len = align4(json_str.size());
    json_str.resize(json_len, ' ');

    size_t bin_len = align4(bin.size());
    ...
    std::cout << "Wrote GLB: " << output_path << "\n";
    return true;
}
...
static bool parse_args(int argc, char** argv, Args& args) {
    for (int i = 1; i < argc; ++i) {
        std::string v = argv[i];
        if (v == "--input-dir" && i + 1 < argc) {
            args.input_dir = argv[++i];
        } else if ((v == "--out" || v == "-o") && i + 1 < argc) {
            args.output_path = argv[++i];
        } else if (v == "--raw-key" && i + 1 < argc) {
            args.raw_key = argv[++i];
        } else if (v == "--ann-key" && i + 1 < argc) {
            args.ann_key = argv[++i];
        } else if (v == "--ann-threshold" && i + 1 < argc) {
            args.ann_threshold = std::stof(argv[++i]);
        } else if (v == "--use-raw-threshold") {
            args.use_raw_threshold = true;
        } else {
            return false;
        }
    }
...
int main(int argc, char** argv) {
    ...
    if (!load_slices(args, raw_volume, ann_volume, z_count, height, width, has_ann)) {
        return 1;
    }

    std::cout << "Loaded volume: Z=" << z_count << " H=" << height << " W=" << width << "\n";
    ...
    if (has_ann) {
        ...
        if (primitives.empty()) {
            std::cerr << "Mesh is empty. Check your annotation or threshold.\n";
            return 1;
        }
    } else {
        if (!args.use_raw_threshold) {
            std::cerr << "No annotation found. Use --use-raw-threshold to build mesh from raw.\n";
            return 1;
        }
    ...
    if (!write_glb(args.output_path, primitives, png)) {
        return 1;
    }
