...
namespace cnpy {
    ...
    template<typename T> void npz_save_fortran(std::string zipname, std::string fname, const T* data, const std::vector<size_t>& shape, std::string mode = "w")
    {
        fname += ".npy";
        ...
        if(fp) {
            ...
            size_t res = fread(&global_header[0],sizeof(char),global_header_size,fp);
            if(res != global_header_size){
                throw std::runtime_error("npz_save_fortran: header read error while adding to existing zip");
            }
            fseek(fp,global_header_offset,SEEK_SET);
        }
        else {
        ...
        std::vector<char> npy_header = create_npy_header<T>(shape, true);
    ...
    template<typename T> std::vector<char> create_npy_header(const std::vector<size_t>& shape) {
        return create_npy_header<T>(shape, false);
    }
