...
inline std::vector<ZipEntry> load_npz_entries(const fs::path& path) {
    ...
    while (offset + 4 <= bytes.size()) {
        ...
        if (sig != 0x04034B50) {
            throw std::runtime_error("NPZ 文件格式异常，无法解析本地 ZIP 头");
        }
        if (offset + 30 > bytes.size()) {
            throw std::runtime_error("NPZ 文件不完整");
        }
        ...
        if ((flags & 0x08U) != 0U) {
            throw std::runtime_error("当前不支持带 data descriptor 的 NPZ 条目");
        }
        ...
        if (method == 0) {
            ...
            throw std::runtime_error("不支持的 ZIP 压缩方法: " + std::to_string(method));
        }
...
inline DTypeInfo parse_dtype(const std::string& descr) {
    if (descr.size() < 3) {
        throw std::runtime_error("不支持的 dtype 描述: " + descr);
    }
    DTypeInfo info;
...
inline NpyMeta parse_npy_meta(const std::vector<uint8_t>& bytes) {
    ...
    if (bytes.size() < prefix) {
        throw std::runtime_error("NPY 头长度不足");
    }
    ...
    if (major == 1) {
        ...
        throw std::runtime_error("不支持的 NPY 版本");
    }

    if (bytes.size() < prefix + header_len) {
        throw std::runtime_error("NPY 头越界");
    }
    ...
    if (descr_pos == std::string::npos || fortran_pos == std::string::npos || shape_pos == std::string::npos) {
        throw std::runtime_error("NPY 头缺少必要字段");
    }
    ...
    while (ss.good()) {
        ...
        if (!digits.empty()) {
            meta.shape.push_back(static_cast<size_t>(std::stoull(digits)));
        }
    }
    if (meta.shape.empty()) {
...
inline std::vector<double> decode_numeric_data(const std::vector<uint8_t>& bytes, const NpyMeta& meta) {
    ...
    if (!dtype.little_endian) {
        throw std::runtime_error("当前仅支持 little-endian NPY");
    }
    ...
    auto copy_typed = [&](auto tag) {
        ...
        const auto* ptr = reinterpret_cast<const T*>(data);
    ...
    if (dtype.kind == 'f') {
        if (dtype.item_size == 4) {
            copy_typed(float{});
        } else if (dtype.item_size == 8) {
            copy_typed(double{});
        } else {
            throw std::runtime_error("不支持的浮点 dtype: " + meta.descr);
        }
    } else if (dtype.kind == 'i') {
        if (dtype.item_size == 1) {
            copy_typed(int8_t{});
        } else if (dtype.item_size == 2) {
            copy_typed(int16_t{});
        } else if (dtype.item_size == 4) {
            copy_typed(int32_t{});
        } else if (dtype.item_size == 8) {
            copy_typed(int64_t{});
        } else {
            throw std::runtime_error("不支持的整数 dtype: " + meta.descr);
        }
        ...
        if (dtype.item_size == 1) {
            ...
            throw std::runtime_error("不支持的无符号 dtype: " + meta.descr);
        }
    } else if (dtype.kind == 'b') {
        copy_typed(uint8_t{});
    } else {
        throw std::runtime_error("不支持的 dtype 类型: " + meta.descr);
    }
    if (meta.fortran_order) {
        return reorder_fortran_to_c(out, meta.shape);
    }
    return out;
}
...
inline std::vector<uint8_t> encode_numeric_data(const std::vector<double>& values, const NpyMeta& meta) {
    ...
    if (values.size() != count) {
        throw std::runtime_error("编码 NPY 数据时元素数量不匹配");
    }
    ...
    if (dtype.kind == 'f') {
        if (dtype.item_size == 4) {
            cast_and_store(float{});
        } else if (dtype.item_size == 8) {
            cast_and_store(double{});
        } else {
            throw std::runtime_error("不支持的浮点 dtype: " + meta.descr);
        }
    } else if (dtype.kind == 'i') {
        if (dtype.item_size == 1) {
            cast_and_store(int8_t{});
        } else if (dtype.item_size == 2) {
            cast_and_store(int16_t{});
        } else if (dtype.item_size == 4) {
            cast_and_store(int32_t{});
        } else if (dtype.item_size == 8) {
            cast_and_store(int64_t{});
        } else {
            throw std::runtime_error("不支持的整数 dtype: " + meta.descr);
        }
    } else if (dtype.kind == 'u') {
        if (dtype.item_size == 1) {
            ...
            cast_and_store(uint16_t{});
        } else if (dtype.item_size == 4) {
            cast_and_store(uint32_t{});
        } else if (dtype.item_size == 8) {
            cast_and_store(uint64_t{});
        } else {
            throw std::runtime_error("不支持的无符号 dtype: " + meta.descr);
        }
    } else if (dtype.kind == 'b') {
...
inline void apply_gamma_inplace(std::vector<double>& values, const DTypeInfo& dtype, double gamma) {
    ...
    if (dtype_is_integer_like(dtype)) {
        ...
        for (double& value : values) {
            const double norm = std::clamp(value / max_value, 0.0, 1.0);
            value = std::pow(norm, gamma) * max_value;
        }
        return;
    }
    ...
    for (double& value : values) {
        const double norm = std::clamp((value - min_value) / range, 0.0, 1.0);
        value = std::pow(norm, gamma) * range + min_value;
    }
}
...
inline cv::Mat process_2d_or_hwc_mat(
    ...
    if (rotate_deg != 0.0) {
        const cv::Point2f center(static_cast<float>(out.cols / 2.0), static_cast<float>(out.rows / 2.0));
        const cv::Mat matrix = cv::getRotationMatrix2D(center, rotate_deg, 1.0);
        cv::warpAffine(out, out, matrix, out.size(), cv::INTER_LINEAR, cv::BORDER_REFLECT);
    }

    if (crop.has_value()) {
        const auto [x, y, cw, ch] = clip_crop_rect(crop->x, crop->y, crop->w, crop->h, out.cols, out.rows);
        out = out(cv::Rect(x, y, cw, ch)).clone();
    }
...
inline cv::Mat process_label_mat(
    ...
    if (rotate_deg != 0.0) {
        const cv::Point2f center(static_cast<float>(out.cols / 2.0), static_cast<float>(out.rows / 2.0));
        const cv::Mat matrix = cv::getRotationMatrix2D(center, rotate_deg, 1.0);
        cv::warpAffine(out, out, matrix, out.size(), cv::INTER_NEAREST, cv::BORDER_REFLECT);
    }
    if (crop.has_value()) {
        const auto [x, y, cw, ch] = clip_crop_rect(crop->x, crop->y, crop->w, crop->h, out.cols, out.rows);
        out = out(cv::Rect(x, y, cw, ch)).clone();
...
inline std::pair<std::vector<double>, std::vector<size_t>> process_image_array(
    ...
    const Args& args) {
    if (shape.size() == 2) {
        cv::Mat mat = make_mat_2d(image, static_cast<int>(shape[0]), static_cast<int>(shape[1]));
        cv::Mat out = process_2d_or_hwc_mat(mat, args.scale_x, args.scale_y, args.rotate_deg, args.crop, args.contrast, args.gamma, dtype);
        if (args.preserve_resolution && (out.rows != static_cast<int>(shape[0]) || out.cols != static_cast<int>(shape[1]))) {
            out = resize_to_shape(out, static_cast<int>(shape[0]), static_cast<int>(shape[1]), cv::INTER_LINEAR);
        }
    ...
    if (shape.size() == 3) {
        if (shape[2] == 1 || shape[2] == 3 || shape[2] == 4) {
            cv::Mat mat = make_mat_hwc(image, static_cast<int>(shape[0]), static_cast<int>(shape[1]), static_cast<int>(shape[2]));
            cv::Mat out = process_2d_or_hwc_mat(mat, args.scale_x, args.scale_y, args.rotate_deg, args.crop, args.contrast, args.gamma, dtype);
            if (args.preserve_resolution &&
                (out.rows != static_cast<int>(shape[0]) || out.cols != static_cast<int>(shape[1]))) {
                out = resize_to_shape(out, static_cast<int>(shape[0]), static_cast<int>(shape[1]), cv::INTER_LINEAR);
            }
            return {mat_to_vector_hwc(out), {static_cast<size_t>(out.rows), static_cast<size_t>(out.cols), shape[2]}};
        }
        ...
        if (shape[0] == 1 || shape[0] == 3 || shape[0] == 4) {
            ...
            std::vector<double> hwc(static_cast<size_t>(height * width * channels), 0.0);
            for (int c = 0; c < channels; ++c) {
                for (int y = 0; y < height; ++y) {
                    for (int x = 0; x < width; ++x) {
                        const size_t src = (static_cast<size_t>(c) * height + static_cast<size_t>(y)) * width + static_cast<size_t>(x);
                        const size_t dst = (static_cast<size_t>(y) * width + static_cast<size_t>(x)) * channels + static_cast<size_t>(c);
                        hwc[dst] = image[src];
                    }
                }
            }

            cv::Mat mat = make_mat_hwc(hwc, height, width, channels);
            cv::Mat out = process_2d_or_hwc_mat(mat, args.scale_x, args.scale_y, args.rotate_deg, args.crop, args.contrast, args.gamma, dtype);
            if (args.preserve_resolution &&
                (out.rows != height || out.cols != width)) {
                out = resize_to_shape(out, height, width, cv::INTER_LINEAR);
            }

            const auto out_hwc = mat_to_vector_hwc(out);
            std::vector<double> chw(static_cast<size_t>(out.rows * out.cols * channels), 0.0);
            for (int c = 0; c < channels; ++c) {
                for (int y = 0; y < out.rows; ++y) {
                    for (int x = 0; x < out.cols; ++x) {
                        const size_t src = (static_cast<size_t>(y) * out.cols + static_cast<size_t>(x)) * channels + static_cast<size_t>(c);
                        const size_t dst = (static_cast<size_t>(c) * out.rows + static_cast<size_t>(y)) * out.cols + static_cast<size_t>(x);
                        chw[dst] = out_hwc[src];
                    }
                }
            }
            return {std::move(chw), {shape[0], static_cast<size_t>(out.rows), static_cast<size_t>(out.cols)}};
        }

        const int depth = static_cast<int>(shape[0]);
        const int height = static_cast<int>(shape[1]);
        const int width = static_cast<int>(shape[2]);
        ...
        for (int i = 0; i < depth; ++i) {
            const size_t start = static_cast<size_t>(i) * height * width;
            const std::vector<double> slice(image.begin() + static_cast<long>(start), image.begin() + static_cast<long>(start + static_cast<size_t>(height * width)));
            cv::Mat mat = make_mat_2d(slice, height, width);
            cv::Mat out = process_2d_or_hwc_mat(mat, args.scale_x, args.scale_y, args.rotate_deg, args.crop, args.contrast, args.gamma, dtype);
            ...
            out_h = out.rows;
            out_w = out.cols;
            const auto vec = mat_to_vector_2d(out);
            result.insert(result.end(), vec.begin(), vec.end());
        }
        return {std::move(result), {static_cast<size_t>(depth), static_cast<size_t>(out_h), static_cast<size_t>(out_w)}};
    }
...
inline std::pair<std::vector<double>, std::vector<size_t>> process_label_array(
    ...
    const Args& args) {
    if (shape.size() == 2) {
        cv::Mat mat = make_mat_2d(label, static_cast<int>(shape[0]), static_cast<int>(shape[1]));
        cv::Mat out = process_label_mat(mat, args.scale_x, args.scale_y, args.rotate_deg, args.crop);
        if (args.preserve_resolution && (out.rows != static_cast<int>(shape[0]) || out.cols != static_cast<int>(shape[1]))) {
            if (args.crop.has_value()) {
                cv::Mat canvas = cv::Mat::zeros(static_cast<int>(shape[0]), static_cast<int>(shape[1]), CV_64F);
                const auto [x, y, cw, ch] = clip_crop_rect(args.crop->x, args.crop->y, args.crop->w, args.crop->h,
                                                           static_cast<int>(shape[1]), static_cast<int>(shape[0]));
                out(cv::Rect(0, 0, std::min(cw, out.cols), std::min(ch, out.rows))).copyTo(
                    canvas(cv::Rect(x, y, std::min(cw, canvas.cols - x), std::min(ch, canvas.rows - y))));
                out = canvas;
            } else {
                out = resize_to_shape(out, static_cast<int>(shape[0]), static_cast<int>(shape[1]), cv::INTER_NEAREST);
            }
        }
        return {mat_to_vector_2d(out), {static_cast<size_t>(out.rows), static_cast<size_t>(out.cols)}};
    }
    ...
    if (shape.size() == 3) {
        if (shape[2] == 1 || shape[2] == 3 || shape[2] == 4) {
            cv::Mat mat = make_mat_hwc(label, static_cast<int>(shape[0]), static_cast<int>(shape[1]), static_cast<int>(shape[2]));
            cv::Mat out = process_label_mat(mat, args.scale_x, args.scale_y, args.rotate_deg, args.crop);
            if (args.preserve_resolution &&
                (out.rows != static_cast<int>(shape[0]) || out.cols != static_cast<int>(shape[1]))) {
                out = resize_to_shape(out, static_cast<int>(shape[0]), static_cast<int>(shape[1]), cv::INTER_NEAREST);
        ...
        for (int i = 0; i < depth; ++i) {
            const size_t start = static_cast<size_t>(i) * height * width;
            const std::vector<double> slice(label.begin() + static_cast<long>(start), label.begin() + static_cast<long>(start + static_cast<size_t>(height * width)));
            cv::Mat mat = make_mat_2d(slice, height, width);
            cv::Mat out = process_label_mat(mat, args.scale_x, args.scale_y, args.rotate_deg, args.crop);
            if (args.preserve_resolution && (out.rows != height || out.cols != width)) {
                out = resize_to_shape(out, height, width, cv::INTER_NEAREST);
...
inline void process_npz_file(const Args& args) {
    ...
    if (!name_to_index.count("image.npy")) {
        throw std::runtime_error("输入 npz 不包含 image 键");
    }
    ...
    if (name_to_index.count("label.npy")) {
        const bool geom_changed = args.scale_x != 1.0 || args.scale_y != 1.0 || args.rotate_deg != 0.0 || args.crop.has_value();
        if (geom_changed) {
    ...
    std::cout << "处理完成: " << output_path << '\n';
    std::cout << "保留原始键:";
    for (const auto& entry : entries) {
        std::cout << ' ' << entry.name.substr(0, entry.name.size() > 4 ? entry.name.size() - 4 : entry.name.size());
    }
    std::cout << '\n';
    std::cout << "image 形状: " << shape_to_string(image_shape) << ", dtype: " << image_meta.descr << '\n';
}

inline std::string usage_text(const std::string& exe_name) {
    std::ostringstream oss;
    oss << "用法:\n"
        << "  " << exe_name << " --input <file.npz> [--output <out.npz>] [--scale-x 1.0] [--scale-y 1.0]\n"
        << "     [--rotate 0] [--crop X Y W H] [--contrast 1.0] [--gamma 1.0] [--preserve-resolution]\n";
    return oss.str();
}
...
inline Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (key == "--input") {
            ...
            args.scale_x = std::stod(require_value(key));
        } else if (key == "--scale-y") {
            args.scale_y = std::stod(require_value(key));
        } else if (key == "--rotate") {
            args.rotate_deg = std::stod(require_value(key));
        } else if (key == "--crop") {
            CropRect rect;
            rect.x = std::stoi(require_value(key));
            rect.y = std::stoi(require_value(key));
            rect.w = std::stoi(require_value(key));
            rect.h = std::stoi(require_value(key));
            args.crop = rect;
        } else if (key == "--contrast") {
            args.contrast = std::stod(require_value(key));
        } else if (key == "--gamma") {
            args.gamma = std::stod(require_value(key));
        } else if (key == "--preserve-resolution") {
            args.preserve_resolution = true;
        } else if (key == "--help" || key == "-h") {
            throw std::runtime_error(usage_text(fs::path(argv[0]).filename().string()));
        } else {
            throw std::runtime_error("未知参数: " + key + "\n" + usage_text(fs::path(argv[0]).filename().string()));
        }
    }

    if (args.input.empty()) {
        throw std::runtime_error("必须提供 --input\n" + usage_text(fs::path(argv[0]).filename().string()));
    }
    return args;
}
